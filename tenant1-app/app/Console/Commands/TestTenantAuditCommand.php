...
class TestTenantAuditCommand extends Command
{
    ...
    {
        $this->info('🧪 Starting Tenant1 Audit System Tests');
        $this->newLine();
    ...
    private function testEnvironmentConfiguration()
    {
        ...
        if ($tenantSlug === 'tenant1') {
            $this->line('  ✓ TENANT_SLUG is correctly configured');
        } else {
            $this->warn("  ⚠️ TENANT_SLUG is '{$tenantSlug}', expected 'tenant1'");
        }
    ...
    private function testDirectLoginFlow()
    {
        ...
        $testUser = User::where('email', 'test-audit@tenant1.com')->first();
        if (!$testUser) {
            $testUser = User::create([
                'name' => 'Test Audit User',
                'email' => 'test-audit@tenant1.com',
        ...
        if (Auth::attempt(['email' => 'test-audit@tenant1.com', 'password' => 'password123'])) {
        ...
        if (!Auth::attempt(['email' => 'test-audit@tenant1.com', 'password' => 'wrongpassword'])) {
    ...
    private function testAuditRecordCreation()
    {
        $this->info('📝 Testing Audit Record Creation...');
        
        // Test direct audit API call to central SSO
        try {
            $response = Http::post(env('CENTRAL_SSO_URL') . '/api/audit/login', [
                'email' => 'audit-test@tenant1.com',
                'tenant_id' => env('TENANT_SLUG'),
                'login_method' => 'api',
                'is_successful' => true,
                'ip_address' => '127.0.0.1',
                'user_agent' => 'Tenant1 Test Script',
                'session_id' => 'test_session_' . uniqid(),
            ]);
            
            if ($response->successful()) {
                $data = $response->json();
                $this->line('  ✓ Audit record created successfully (ID: ' . $data['audit_id'] . ')');
            } else {
                $this->error('  ❌ Failed to create audit record: HTTP ' . $response->status());
            }
        } catch (\Exception $e) {
            $this->error('  ❌ Error creating audit record: ' . $e->getMessage());
        }
        
        $this->info('  ✅ Audit record creation tests completed');
        $this->newLine();
    }
}