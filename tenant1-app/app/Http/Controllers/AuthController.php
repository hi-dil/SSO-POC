...
class AuthController extends Controller
{
    ...
    public function ssoCallback(Request $request)
    {
        ...
        if ($result['valid']) {
            ...
            // Find or create/update local user based on SSO user data
            $localUser = \App\Models\User::updateOrCreate(
                ['email' => $ssoUser['email']],
                [
                    'name' => $ssoUser['name'],
            ...
            // Store JWT token and user data for future API calls to central SSO
            session(['jwt_token' => $token]);
            session(['sso_user_data' => $ssoUser]);
    ...
    public function dashboard()
    {
        ...
        // Use cached SSO user data if available, otherwise use local user data
        $ssoUserData = session('sso_user_data');
        
        if ($ssoUserData) {
            // Use the rich SSO user data
            $user = [
                'id' => $localUser->id, // Local user ID for UI
                'sso_id' => $ssoUserData['id'], // Central SSO ID
                'name' => $ssoUserData['name'],
                'email' => $ssoUserData['email'],
                'current_tenant' => env('TENANT_SLUG'),
                'tenants' => $ssoUserData['tenants'] ?? [env('TENANT_SLUG')],
                'is_admin' => $ssoUserData['is_admin'] ?? false
            ];
        } else {
            // Fallback to local user data (for users who haven't been through SSO recently)
            $user = [
                'id' => $localUser->id,
                'name' => $localUser->name,
            ...
            // Try to refresh from JWT if available
            $jwtToken = session('jwt_token');
            if ($jwtToken) {
                $result = $this->ssoService->validateToken($jwtToken);
                if ($result['valid'] && isset($result['user'])) {
                    $ssoUser = $result['user'];
                    $user['tenants'] = $ssoUser['tenants'] ?? [env('TENANT_SLUG')];
                    $user['is_admin'] = $ssoUser['is_admin'] ?? false;
                    // Update cached data
                    session(['sso_user_data' => $ssoUser]);
                }
            }
