...
function submitForm() {
    ...
    let url;
    if (isEdit) {
        url = '{{ route('admin.users.addresses.update', [$user, ':id']) }}'.replace(':id', addressId);
    } else {
        url = '{{ route('admin.users.addresses.store', $user) }}';
    }
...
function confirmDelete(addressId) {
    ...
    if (confirm(`Are you sure you want to delete this address: ${address.address_line_1}?`)) {
        ...
        form.action = '{{ route('admin.users.addresses.destroy', [$user, ':id']) }}'.replace(':id', addressId);
        form.submit();
