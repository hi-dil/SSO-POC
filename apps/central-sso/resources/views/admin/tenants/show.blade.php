...
@section('actions')
    <div class="flex space-x-3">
        @can('tenants.view')
            <a href="{{ route('admin.tenants.users', $tenant) }}" class="inline-flex items-center px-4 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
        ...
        @endcan
        @can('tenants.edit')
            <a href="{{ route('admin.tenants.edit', $tenant) }}" class="inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 dark:bg-indigo-500 hover:bg-indigo-700 dark:hover:bg-indigo-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                <svg class="-ml-1 mr-2 h-5 w-5" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div class="px-4 py-5 sm:p-4">
                <dl class="sm:divide-y sm:divide-gray-200 dark:sm:divide-gray-700">
                    ...
                    <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4">
                        ...
                        <dd class="mt-1 text-sm text-gray-900 dark:text-white sm:mt-0 sm:col-span-2">
                            @if($tenant->domain)
                                <a href="https://{{ $tenant->domain }}" target="_blank" class="text-indigo-600 dark:text-indigo-400 hover:text-indigo-500 dark:hover:text-indigo-300">
                                    {{ $tenant->domain }}
                    ...
                    <div class="py-4 sm:py-5 sm:grid sm:grid-cols-3 sm:gap-4">
                        ...
                        <dd class="mt-1 text-sm text-gray-900 dark:text-white sm:mt-0 sm:col-span-2">
                            ...
                            <span class="text-gray-500 dark:text-gray-400">({{ $tenant->created_at->diffForHumans() }})</span>
                        </dd>
                    </div>
                </dl>
            </div>
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow rounded-lg border border-gray-200 dark:border-gray-700">
                <div class="p-5">
                    <div class="flex items-center">
                        ...
                        <div class="ml-5 w-0 flex-1">
                            <dl>
                                <dt class="text-sm font-medium text-gray-500 dark:text-gray-400 truncate">Last Activity</dt>
    ...
    @if($tenant->users->count() > 0)
        <div class="mb-8">
            <div class="flex justify-between items-center mb-4">
                ...
                @can('tenants.view')
                    <a href="{{ route('admin.tenants.users', $tenant) }}" class="text-sm text-indigo-600 dark:text-indigo-400 hover:text-indigo-500 dark:hover:text-indigo-300">
                        View all users →
                    </a>
                @endcan
            </div>
        ...
        <div class="border-t border-gray-200 dark:border-gray-700 pt-6">
            ...
            <div class="flex flex-wrap gap-3">
                <form method="POST" action="{{ route('admin.tenants.toggle', $tenant) }}" class="inline">
                    @csrf
                    @method('PATCH')
                    <button type="submit" class="inline-flex items-center px-3 py-2 border border-gray-300 dark:border-gray-600 shadow-sm text-sm leading-4 font-medium rounded-md text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 hover:bg-gray-50 dark:hover:bg-gray-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
